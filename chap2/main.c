...
int main(int argc, char **argv)
{
	...
	while(true)
	{
		...
		temperatureMeasured_fractional = ((inputRegister >> 10) & 0x00000007); // Assignment
		// bits 13 to 17 are decimal part
		unsigned int temperatureMeasured_decimal = 0; // Assignment
		temperatureMeasured_decimal = ((inputRegister >> 13) & 0x0000001F); // Assignment
		// According to equation 2:
			// Assignment
			// Assignment
		temperatureMeasured = (float)temperatureMeasured_decimal * 1.0 + (float)temperatureMeasured_fractional*0.2; // Assignment
			if (temperatureMeasured > 30) temperatureMeasured = 31;
			if (temperatureMeasured_fractional == 0 && temperatureMeasured_decimal == 31) temperatureMeasured = -1;
			// Assignment
			// Assignment
		//------We Added----------
		...
		if (isAcOn)
		{
			...
			if((temperatureDesired - temperatureMeasured) > Hysteresis_Heater)
			{
				isCoolerOn = false;
				isHeaterOn = true;
			}
		...
		if ((isCoolerOn && (temperatureMeasured <= temperatureDesired)) || isHeaterOn && (temperatureMeasured >= temperatureDesired))
		{
			isCoolerOn = false;
			isHeaterOn = false;
		}
		...
		outputRegister |= (isCoolerOn << 4) ;// Assignment
		...
		outputRegister |= (isHeaterOn << 5) ;// Assignment
